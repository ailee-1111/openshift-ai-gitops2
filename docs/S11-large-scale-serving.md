...
> **결과**: 4/4 항목 검증 완료 — 122B MoE 모델 TP=2 분산 서빙(TTFT 253ms, 13.0 tok/s), 6개 이기종 모델 동시 서빙(H200 8/8 GPU), bfloat16/FP8 이중 정밀도 확인. 대규모 AI 서빙 플랫폼의 GPU 활용 효율성과 모델 다양성을 실증하여 Mobis 자율주행 추론 워크로드의 확장 가능성을 검증했다.
>
...
```bash
oc apply -f infra/poc/model-serving/qwen35-122b-llmis.yaml -n mobis-poc
```
...
NAME                             URL                                                                      READY
redhataiqwen35-122b-a10b-fp8-d   http://maas.apps.poc.mobis.com/mobis-poc/redhataiqwen35-122b-a10b-fp8-d   True
```
...
| 엔드포인트 | https://maas.apps.poc.mobis.com/mobis-poc/redhataiqwen35-122b-a10b-fp8-d |
...
2. DataConnection `mobis-s3-connection` — S3(MinIO) 접근용
3. S3 모델 아티팩트 업로드 완료:
...
```bash
oc apply -f infra/poc/model-serving/gemma-4-31b.yaml -n mobis-poc
oc apply -f infra/poc/model-serving/qwen3-30b.yaml -n mobis-poc
oc apply -f infra/poc/model-serving/qwen3-vl-8b-fp8.yaml -n mobis-poc
```
...
```
$ oc get inferenceservice -n mobis-poc \
...
gemma-4-31b-it-rh             True    https://gemma-4-31b-it-rh-mobis-poc.apps.poc.mobis.com
qwen3-30b-a3b-instruct-2507   True    https://qwen3-30b-a3b-instruct-2507-mobis-poc.apps.poc.mobis.com
qwen3-vl-8b-instruct-fp8      True    https://qwen3-vl-8b-instruct-fp8-mobis-poc.apps.poc.mobis.com
```
...
```
$ oc get inferenceservice qwen3-30b-a3b-instruct-2507 -n mobis-poc \
    -o jsonpath='{.spec.predictor.model.args}'
...
| Operator | 채널 | 버전 | 비고 |
|----------|------|------|------|
| Model Registry Operator | alpha | 최신 | mobis-registry 인스턴스 생성 |
| RHOAI | fast | 2.22 | KServe + LLMIS CRD |

**사전 생성 리소스**:

1. ModelRegistry 인스턴스 `mobis-registry` — `rhoai-model-registries` 네임스페이스
...
4. DataConnection (mobis-s3-connection, poc-s3-connection)
...
bge-m3-v1                     True    <none>    https://bge-m3-v1-mobis-poc.apps.poc.mobis.com
bge-reranker-v2-m3            True    <none>    https://bge-reranker-v2-m3-mobis-poc.apps.poc.mobis.com
gemma-4-31b-it-rh             True    <none>    https://gemma-4-31b-it-rh-mobis-poc.apps.poc.mobis.com
qwen3-30b-a3b-instruct-2507   True    <none>    https://qwen3-30b-a3b-instruct-2507-mobis-poc.apps.poc.mobis.com
qwen3-vl-8b-instruct-fp8      True    <none>    https://qwen3-vl-8b-instruct-fp8-mobis-poc.apps.poc.mobis.com
...
```
$ oc get llminferenceservice -n mobis-poc \
    -o custom-columns='NAME:.metadata.name,READY:.status.conditions[?(@.type=="Ready")].status,URL:.status.url'
NAME                             READY   URL
qwen3-8b                         False   http://maas.apps.poc.mobis.com/mobis-poc/qwen3-8b
redhataiqwen3-30b-a3b-speculat   False   http://maas.apps.poc.mobis.com/mobis-poc/redhataiqwen3-30b-a3b-speculat
redhataiqwen35-122b-a10b-fp8-d   True    http://maas.apps.poc.mobis.com/mobis-poc/redhataiqwen35-122b-a10b-fp8-d
```
...
```
$ oc get pods -n mobis-poc -o custom-columns='NAME:.metadata.name,NODE:.spec.nodeName,GPU:.spec.containers[0].resources.limits.nvidia\.com/gpu,STATUS:.status.phase' \
    | grep Running | grep -v '<none>'
NAME                                                              NODE                     GPU      STATUS
bge-m3-v1-predictor-6765745d6d-qnts7                              master01.poc.mobis.com   1        Running
bge-reranker-v2-m3-predictor-64cf6855bf-cdg6g                     master01.poc.mobis.com   1        Running
gemma-4-31b-it-rh-predictor-5b46bb6c66-whwv7                      master01.poc.mobis.com   1        Running
qwen3-30b-a3b-instruct-2507-predictor-6b4f889ddf-kk6qc            master01.poc.mobis.com   1        Running
qwen3-vl-8b-instruct-fp8-predictor-6c7c45c5d7-vlv64               master01.poc.mobis.com   1        Running
redhataiqwen35-122b-a10b-fp8-d-kserve-fc894f8f5-t8qgw             master01.poc.mobis.com   2        Running
smollm2-s5-zero-predictor-58f8b446f9-gwfnn                        master01.poc.mobis.com   1        Running
```

```
$ oc get pdb -n mobis-poc
No resources found in mobis-poc namespace.
```

```
$ oc get pods -n mobis-poc -o jsonpath='{range .items[?(@.spec.containers[0].resources.limits.nvidia\.com/gpu)]}{.metadata.name}{"\taffinity="}{.spec.affinity}{"\n"}{end}'
...
master01.poc.mobis.com   NVIDIA-H200   8           8
worker01.poc.mobis.com   NVIDIA-A40    2           2
```
...
> ⚠️ 미해결:
>
> 1. **단일 노드 집중 배치**: 모든 GPU 서빙 Pod가 `master01.poc.mobis.com`(H200 x8)에 집중되어 있다. Anti-affinity 룰 미설정, PodDisruptionBudget 미생성. master01 노드 장애 시 전체 서빙 중단.
>
...
```bash
oc patch inferenceservice smollm2-135m -n mobis-poc --type=merge -p '{
  "spec": {
    "predictor": {
      "model": {
        "storage": {
...
> oc patch inferenceservice smollm2-135m -n mobis-poc --type=merge -p '{"spec":{"predictor":{"minReplicas":1}}}'
> ```
...
>    oc get servingruntime vllm-cuda-runtime -n mobis-poc \
...
>    ```bash
>    # Gemma-4-31B 예시
>    oc patch inferenceservice gemma-4-31b-it-rh -n mobis-poc --type=merge -p '{
>      "spec": {
>        "predictor": {
>          "model": {
...
>    ```bash
>    oc delete servingruntime vllm-upstream-nightly-test -n mobis-poc
>    ```
