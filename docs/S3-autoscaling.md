...
CMA/KEDA 기반 수평 오토스케일링 3개 검증 항목을 모두 PASS(1 조건부) 판정하였다. 스케일업 14초(1→3 replicas), DCGM 10 targets + vLLM 6 targets 메트릭 수집 정상, v3 advanced HPA behavior 커스터마이징 적용-검증-복원을 완료하여 Mobis의 LLM 서빙 부하 대응 자동화 요건을 충족한다.
...
- 대상 InferenceService(smollm2-135m)가 mobis-poc 네임스페이스에 배포되어 Running 상태
...
```bash
# MODEL_NAME, MODEL_NS 는 환경변수로 사전 설정 필요 (예: smollm2-135m, mobis-poc)
...
oc create sa keda-prometheus-reader -n mobis-poc
oc adm policy add-cluster-role-to-user cluster-monitoring-view \
  -z keda-prometheus-reader -n mobis-poc

# 장기 토큰 Secret 생성
oc create token keda-prometheus-reader -n mobis-poc --duration=87600h > /tmp/keda-token
oc create secret generic keda-prometheus-auth -n mobis-poc \
  ...
  triggers:
    - type: prometheus
      metadata:
        ...
        query: |
          ( sum(vllm:num_requests_running{namespace="mobis-poc", model_name="smollm2-135m"})
          + sum(vllm:num_requests_waiting{namespace="mobis-poc", model_name="smollm2-135m"}) )
          OR on() vector(0)
        threshold: "2"
        unsafeSsl: "true"
        authModes: "bearer"
    ...
    spec:
      containers:
        - name: loader
          ...
          args:
            - |
              ROUTE="smollm2-135m-mobis-poc.apps.mobis.ocp.com"
...
```bash
# 부하 테스트 실행
oc apply -f vllm-load-test.yaml -n mobis-poc

# 스케일업 관찰 (10초 간격)
watch -n 10 'oc get hpa keda-hpa-vllm-autoscaler -n mobis-poc --no-headers; echo "---"; oc get pods -n mobis-poc -l serving.kserve.io/inferenceservice=smollm2-135m --no-headers'

# 완료 후 정리
oc delete job vllm-load-test -n mobis-poc
```
...
> 📸 **재촬영 필요**: ScaledObject READY=True 상태 스크린샷 — URL: OpenShift Console > Installed Operators > Custom Metrics Autoscaler > ScaledObject 탭 > `vllm-autoscaler` 상세, 조건: smollm2-135m InferenceService를 `oc apply -f infra/poc/models/smollm2-135m.yaml -n mobis-poc`로 재배포 후 predictor Pod가 Running 상태에 도달하면 ScaledObject가 READY=True로 전환된다. 이 시점에 (1) 콘솔 ScaledObjects 목록에서 READY=True 행 캡처, (2) 터미널에서 `oc get scaledobject vllm-autoscaler -n mobis-poc` 출력 캡처. 파일명: `S3-scaledobject-ready-true.png`

> 📸 **재촬영 필요**: HPA scale-up 터미널 캡처 (1→2→3 replicas) — URL: 터미널 세션, 조건: 위 모델 재배포 완료 후 부하 테스트 Job(`vllm-load-test.yaml`)을 적용하고, 아래 watch 명령어로 관찰한다. REPLICAS 컬럼이 1→2→3으로 증가하는 과정이 표시되는 시점에 터미널 전체를 캡처한다. `watch -n 5 'oc get hpa keda-hpa-vllm-autoscaler -n mobis-poc; echo ---; oc get pods -n mobis-poc -l serving.kserve.io/inferenceservice=smollm2-135m'`. 파일명: `S3-hpa-scaleup-terminal.png`
...
```bash
oc get scaledobject vllm-autoscaler -n mobis-poc
```
...
```bash
oc describe scaledobject vllm-autoscaler -n mobis-poc
```
...
Spec:
  ...
  Triggers:
    ...
    Metadata:
      Auth Modes:      bearer
      Query:           ( sum(vllm:num_requests_running{namespace="mobis-poc", model_name="smollm2-135m"}) + sum(vllm:num_requests_waiting{namespace="mobis-poc", model_name="smollm2-135m"}) ) OR on() vector(0)
...
```bash
oc get hpa keda-hpa-vllm-autoscaler -n mobis-poc
```
...
```bash
oc describe hpa keda-hpa-vllm-autoscaler -n mobis-poc
```
...
> 📸 **재촬영 필요**: ScaledObject READY=True 상태 스크린샷 — URL: `https://console-openshift-console.apps.mobis.ocp.com/k8s/ns/mobis-poc/keda.sh~v1alpha1~ScaledObject/vllm-autoscaler`, 조건: smollm2-135m InferenceService 재배포 후 predictor Pod Running 상태에서 ScaledObject READY=True 전환 확인 시 캡처. 절차: (1) `oc apply -f infra/poc/models/smollm2-135m.yaml -n mobis-poc` (2) `oc wait --for=condition=Ready pod -l serving.kserve.io/inferenceservice=smollm2-135m -n mobis-poc --timeout=120s` (3) `oc get scaledobject vllm-autoscaler -n mobis-poc`에서 READY=True 확인 후 콘솔 페이지 캡처. 파일명: `S3-scaledobject-ready-true.png`

> 📸 **재촬영 필요**: HPA scale-up 터미널 캡처 — URL: 터미널 세션 (SSH 또는 로컬), 조건: 모델 재배포 + READY=True 확인 후, (1) 터미널 A에서 `watch -n 5 'oc get hpa keda-hpa-vllm-autoscaler -n mobis-poc; echo ---; oc get pods -n mobis-poc -l serving.kserve.io/inferenceservice=smollm2-135m --no-headers'` 실행, (2) 터미널 B에서 `oc apply -f vllm-load-test.yaml -n mobis-poc` 부하 주입, (3) 터미널 A에서 REPLICAS 1→2→3 변화가 보이는 시점에 전체 화면 캡처. 파일명: `S3-hpa-scaleup-terminal.png`
...
아래는 런북 `runbooks/322-autoscaling-v3.md`에서 정의한 v3 강화 ScaledObject 설정이다. `${MODEL_NS}`, `${MODEL_NAME}`은 런북 실행 시 환경변수로 치환되는 값이며, 정적 YAML 적용 시에는 각각 `mobis-poc`, `smollm2-135m`으로 대체한다.
  ...
  triggers:
    - type: prometheus
      metadata:
        serverAddress: "https://thanos-querier.openshift-monitoring.svc.cluster.local:9091"
        query: |
          sum(vllm:num_requests_running{namespace="mobis-poc"}) +
          sum(vllm:num_requests_waiting{namespace="mobis-poc"})
        threshold: "2"
        unsafeSsl: "true"
        authModes: "bearer"
      authenticationRef:
...
```bash
# v3 강화 적용
oc apply -f runbooks/322-autoscaling-v3-scaledobject.yaml -n mobis-poc
...
> 📸 **재촬영 필요**: v3 HPA behavior 적용 상태의 `oc describe hpa` 출력 — URL: 터미널 세션, 조건: (1) 대상 모델 InferenceService가 Running 상태 (2) v3 ScaledObject YAML을 `oc apply -f runbooks/322-autoscaling-v3-scaledobject.yaml -n mobis-poc`로 적용 (3) `oc get scaledobject vllm-autoscaler -n mobis-poc`에서 READY=True 및 TRIGGERS=prometheus,prometheus (이중 트리거) 확인 (4) `oc describe hpa keda-hpa-vllm-autoscaler -n mobis-poc` 실행하여 Behavior 섹션에서 `scaleUp.stabilizationWindowSeconds: 30`, `scaleDown.stabilizationWindowSeconds: 120`, policies 항목이 표시되는 출력 전체 캡처 (5) 캡처 후 `oc apply -f infra/poc/autoscaling/scaledobject-vllm.yaml`로 v1 복원. 파일명: `S3-hpa-v3-behavior.png`
...
```bash
oc get scaledobject vllm-autoscaler -n mobis-poc -o jsonpath='{.spec}' | python3 -m json.tool
```
...
```bash
oc get triggerauthentication keda-prometheus-creds -n mobis-poc
```
...
|------------|----------|-----------|----------------|-----------|
| ScaledObject READY=True 상태 | No.21 조건부 PASS | smollm2-135m 또는 신규 모델 재배포 후 predictor Pod Running | `oc get scaledobject -n mobis-poc` + 콘솔 ScaledObject 상세 페이지 캡처 | `S3-scaledobject-ready-true.png` |
