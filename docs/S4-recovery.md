...
```bash
$ oc get pods -n mobis-poc -l component=predictor \
...
![S4 Pod 목록](screenshots/S4-pods-mobis-poc.png)
...
| **환경변수** | `MODEL_NS=mobis-poc`, `MODEL_NAME=smollm2-135m` (경량 모델 기준) |
...
> 📸 재촬영 필요: [gemma-4-31b MTTR 실측 후 Pod 이벤트 로그] [Pod 삭제 → Ready 전환 과정] [oc get events -n mobis-poc --sort-by='.lastTimestamp']
...
$ oc describe node master01.poc.mobis.com | grep -A5 "Allocated resources:"
Allocated resources:
...
$ oc describe node worker01.poc.mobis.com | grep -A5 "Allocated resources:"
Allocated resources:
  Resource                Requests        Limits
  --------                --------        ------
...
bge-reranker-v2-m3-predictor-64cf6855bf-cdg6g            master01.poc.mobis.com   Running   True   1   0
gemma-4-31b-it-rh-predictor-5b46bb6c66-whwv7             master01.poc.mobis.com   Running   True   1   0
qwen3-30b-a3b-instruct-2507-predictor-6b4f889ddf-kk6qc   master01.poc.mobis.com   Running   True   1   0
qwen3-vl-8b-instruct-fp8-predictor-6c7c45c5d7-vlv64      master01.poc.mobis.com   Running   True   1   0
```
...
![S4 Pod 노드 배치](screenshots/S4-pods-mobis-poc.png)
...
$ oc get deployment gemma-4-31b-it-rh-predictor -n mobis-poc \
...
```bash
$ oc get routes -n mobis-poc \
    -o custom-columns="NAME:.metadata.name,HOST:.spec.host,SERVICE:.spec.to.name,TLS:.spec.tls.termination" \
    | grep -E "gemma|qwen|bge"
bge-reranker-v2-m3            bge-reranker-v2-m3-mobis-poc.apps.poc.mobis.com            bge-reranker-v2-m3-predictor            edge
gemma-4-31b-it-rh             gemma-4-31b-it-rh-mobis-poc.apps.poc.mobis.com             gemma-4-31b-it-rh-predictor             edge
qwen3-30b-a3b-instruct-2507   qwen3-30b-a3b-instruct-2507-mobis-poc.apps.poc.mobis.com   qwen3-30b-a3b-instruct-2507-predictor   edge
qwen3-vl-8b-instruct-fp8      qwen3-vl-8b-instruct-fp8-mobis-poc.apps.poc.mobis.com      qwen3-vl-8b-instruct-fp8-predictor      edge
```
...
| **현재 상태** | IaC 정의는 존재하나 **mobis-poc 네임스페이스에 미배포** |
...
```bash
# ArgoCD 동기화 또는 수동 적용
oc apply -k infra/poc/network/ -n mobis-poc
```
...
```bash
$ oc get networkpolicy -n mobis-poc \
...
$ oc get networkpolicy deny-from-other-namespaces -n mobis-poc 2>&1
Error from server (NotFound): networkpolicies.networking.k8s.io "deny-from-other-namespaces" not found

$ oc get networkpolicy allow-from-ingress -n mobis-poc 2>&1
Error from server (NotFound): networkpolicies.networking.k8s.io "allow-from-ingress" not found
```

> IaC(`infra/poc/network/`)에 4개 기본 NetworkPolicy가 정의되어 있으나 mobis-poc 네임스페이스에 배포되지 않은 상태. ArgoCD Application 동기화 또는 수동 `oc apply -k` 필요.
...
| 우선순위 | 작업 | 상세 |
|----------|------|------|
| **P1** | 기본 NetworkPolicy 배포 | `oc apply -k infra/poc/network/ -n mobis-poc` — deny-from-other-namespaces + allow-from-ingress |
...
> 📸 재촬영 필요: [NetworkPolicy 배포 후 oc get networkpolicy -n mobis-poc 전체 목록] [서빙 Pod 접근 차단 테스트 결과 (curl from 다른 네임스페이스)] [OpenShift Console > Networking > NetworkPolicies 화면]
...
|----------|------|----------|----------|------|
| **P0** | 기본 네임스페이스 격리 | IaC 정의 완료, 미배포 | `oc apply -k infra/poc/network/ -n mobis-poc` | 프로덕션 전 필수 |
