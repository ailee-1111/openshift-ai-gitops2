...
PASS 30/30 (100%) -- 플랫폼 운영관리 전 항목 검증 완료. LDAP/AD 인증 연동, RBAC 3단계 역할 분리, GPU 10개 실시간 모니터링, Perses 대시보드 9개, 알림 5개 규칙 등록으로 AI 플랫폼 운영 기반을 확보하였다. 이를 통해 Mobis AI 플랫폼의 Day-2 운영 자동화 및 가시성을 확보하고, 수동 모니터링 대비 GPU 이상 탐지 시간을 분 단위로 단축할 수 있다.
...
> **결과**: PASS 30/30 (100%)
> **변경 이력**: 초기 No.45~46 SKIP(28/30) → 고객 AD(gjldap.mobis.co.kr) 실연동 완료, 전체 PASS 전환.
...
#### 검증 패턴
모비스 실제 AD 서버(gjldap.mobis.co.kr)를 OpenShift OAuth LDAP Identity Provider로 연동하여, AD 사용자 계정으로 OpenShift 콘솔 및 RHOAI Dashboard 로그인이 가능함을 확인한다. No.46(AD 연동)은 동일 LDAP IdP 인프라로 동시 충족. 검증 기준:
...
> **운영 참고** -- LDAPS 마이그레이션 절차: (1) AD 팀에서 CA 인증서 번들(PEM) 수령, (2) `oc create configmap ldap-ca --from-file=ca.crt=<ca-bundle.pem> -n openshift-config`, (3) OAuth CR의 ldap 섹션에 `ca: { name: ldap-ca }` 추가 + `url`을 `ldaps://gjldap.mobis.co.kr:636/...`로 변경 + `insecure: false` 설정, (4) OAuth Pod 롤링 재시작 후 로그인 테스트.
...
#### 판정
**PASS** -- 고객 AD(gjldap.mobis.co.kr) 연동 완료. 294개 그룹 동기화, Service_rhoai 사용자 인증 동작 확인. No.46(AD 연동)은 동일 LDAP IdP로 동시 충족.
...
2. **RoleBinding 생성**: 각 사용자에 대한 RoleBinding을 `mobis-poc` 네임스페이스에 생성
...
```bash
# 전체 관리 (admin)
oc adm policy add-role-to-user admin poc-admin -n mobis-poc

# 리소스 편집 (edit)
oc adm policy add-role-to-user edit poc-operator -n mobis-poc
...
#### 검증 결과 (2026-06-09)
```bash
$ oc get rolebinding -n mobis-poc -o custom-columns='NAME:.metadata.name,ROLE:.roleRef.name,USER:.subjects[*].name' \
...
> ⚠️ PoC 제약: ResourceQuota에 GPU 자원 제한(`requests.nvidia.com/gpu`)이 미포함. PoC 환경에서는 GPU 워크로드가 `mobis-poc` 네임스페이스에서만 실행되므로 수용. 운영 전환 시 팀별 네임스페이스에 GPU quota 추가 필수 -- 미설정 시 단일 팀이 전체 GPU를 점유하여 멀티테넌시 격리가 무효화됨. 예: `spec.hard: { "requests.nvidia.com/gpu": "2" }`를 ResourceQuota에 추가하면 해당 네임스페이스 내 전체 Pod의 GPU 합산이 2를 초과할 수 없다.
...
```
NAME                      GPU
master01.poc.mobis.com     8
worker01.poc.mobis.com     2
```
...
mobis-poc 활성 타겟: 10개 (전체 health=up)
...
spec:
  ...
  - emailConfigs:
    - from: rhoai@mobis.com
      ...
      to: dt007000@mobisdev-partners.com
    name: smtp-email
  route:
    groupBy:
    - alertname
...
**알림 흐름**:
```
PrometheusRule 발화 → Alertmanager → 이메일(dt007000@mobisdev-partners.com)
```
...
#### 검증 결과 (2026-06-09)
```bash
$ oc get prometheusrule -n mobis-poc --no-headers
```
```
poc-gpu-alerts   22d
vllm-alerts      22d
...
> 📸 재촬영 필요: [vLLM Pod access log 화면을] [요청 처리 중 상태에서] [oc logs <pod-name> -n mobis-poc | grep "POST /v1/chat"]
...
#### 검증 결과 (2026-06-09)
```bash
$ oc get events -n mobis-poc --field-selector involvedObject.kind=InferenceService \
...
```
NAME                      CPU       MEM              GPU
master01.poc.mobis.com    191500m   1574340164Ki     8
worker01.poc.mobis.com    94800m    252951424Ki      2
```
...
```
```
rhods-dashboard-redhat-ods-applications.apps.poc.mobis.com
```
...
> 📸 재촬영 필요: [RHOAI Dashboard 메인 화면을] [로그인 후 상태에서] [https://rhods-dashboard-redhat-ods-applications.apps.poc.mobis.com]
...
#### 검증 결과 (2026-06-09)
```bash
$ oc get inferenceservice -n mobis-poc --no-headers | head -5
```
```
...
$ oc get inferenceservice -n mobis-poc -o json | python3 -c "import sys,json; print(json.load(sys.stdin)['apiVersion'])"
```
```
...
$ oc logs deploy/gemma-4-31b-it-rh-predictor -n mobis-poc | grep "Avg prompt throughput" | tail -1
```
```
...
> 📸 재촬영 필요: [vLLM 엔진 로그에서 throughput/Running/Waiting 라인을] [모델 서빙 중 상태에서] [oc logs deploy/gemma-4-31b-it-rh-predictor -n mobis-poc]
...
| **P1** | 알림 채널 | 이메일(dt007000@mobisdev-partners.com) | SIEM/SOC 통합 (Splunk/ELK) + PagerDuty/Slack | 2일 |
