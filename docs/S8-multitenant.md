...
4. **모델 배포 완료**
   - InferenceService Running 상태 (customer-poc 네임스페이스)
...
for TEAM in team-a team-b; do
  oc apply -n customer-poc -f infra/poc/multitenant/api-keys/${TEAM}-api-key.yaml
done
```
...
```
$ oc get tokenratelimitpolicies -n customer-poc --no-headers
fallback-trlp                              7d11h
...
```
$ oc get tokenratelimitpolicy -n customer-poc maas-trlp-redhataiqwen35-122b-a10b-fp8-d \
...
> - URL: `https://rh-ai.apps.poc.customer.com/ai-hub/models/deployments/subscriptions`
...
> - URL: `https://rh-ai.apps.poc.customer.com/ai-hub/models/deployments/subscriptions` > API Key 탭
...
> - URL: `https://rh-ai.apps.poc.customer.com/ai-hub/models/deployments/subscriptions` > 티어 필터
...
> - URL: `https://rh-ai.apps.poc.customer.com/ai-hub/models/deployments/subscriptions` > 사용량 탭
...
3. **HTTPRoute + TokenRateLimitPolicy 바인딩**
   - HTTPRoute: `redhataiqwen35-122b-a10b-fp8-d-kserve-route` (customer-poc)
   - HTTPRoute: `fallback-model-routing` (customer-poc)
...
metadata:
  ...
  labels:
    ...
    maas.opendatahub.io/model-namespace: customer-poc
spec:
  limits:
    models-as-a-service-poc-test-redhataiqwen35-122b-a10b-fp8-d-tokens:
      ...
      - predicate: >-
          ...
          "models-as-a-service/poc-test@customer-poc/redhataiqwen35-122b-a10b-fp8-d"
    ...
    models-as-a-service-team-redhataiqwen35-122b-a10b-fp8-d-tokens:
      ...
      - predicate: >-
          auth.identity.selected_subscription_key ==
          "models-as-a-service/team@customer-poc/redhataiqwen35-122b-a10b-fp8-d"
          && !request.path.endsWith("/v1/models")
    models-as-a-service-max-redhataiqwen35-122b-a10b-fp8-d-tokens:
      ...
      - predicate: >-
          auth.identity.selected_subscription_key ==
          "models-as-a-service/max@customer-poc/redhataiqwen35-122b-a10b-fp8-d"
          && !request.path.endsWith("/v1/models")
    models-as-a-service-pro-redhataiqwen35-122b-a10b-fp8-d-tokens:
      ...
      - predicate: >-
          auth.identity.selected_subscription_key ==
          "models-as-a-service/pro@customer-poc/redhataiqwen35-122b-a10b-fp8-d"
          && !request.path.endsWith("/v1/models")
  targetRef:
...
```bash
oc apply -f infra/poc/multitenant/trlp/fallback-trlp.yaml -n customer-poc
```
...
```
$ oc get tokenratelimitpolicy -n customer-poc fallback-trlp \
    -o jsonpath='{range .status.conditions[*]}{.type}={.status} {.lastTransitionTime}{"\n"}{end}'
...
$ curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer ${TEAM_B_KEY}" \
    "https://maas-gateway.apps.poc.customer.com/v1/chat/completions" \
...
> ⚠️ **PoC 제약**: Egress NetworkPolicy가 미적용 상태이다. 현재 Ingress-only 정책(`deny-from-other-namespaces`)만 적용되어, 타 네임스페이스에서의 인바운드 접근은 차단되지만 Pod에서 외부/타 네임스페이스로의 아웃바운드 통신은 제한 없이 허용된다. PoC 환경에서는 모델 서빙 접근(`customer-poc`), 외부 레지스트리 Pull, S3 스토리지 접근 등이 필요하여 Egress 정책을 의도적으로 미적용하였다. **프로덕션 전환 시 `restrict-egress` NetworkPolicy 추가 필수** (데이터 유출 방지). 상세 YAML은 **보안 개선 사항 섹션** 참조.
>
...
>   - to:
>     - podSelector: {}
>   # 모델 서빙 네임스페이스 (customer-poc) 접근 허용
>   - to:
>     - namespaceSelector:
>         matchLabels:
...
2. **vLLM 모델 배포 + ServiceMonitor**
   - 12개 ServiceMonitor 활성 (customer-poc)
...
3. **MaaS Alerting Stack 구성**
   ...
   - PrometheusRule: `poc-gpu-alerts`, `vllm-alerts` (customer-poc)
...
oc apply -f infra/poc/monitoring/poc-gpu-alerts.yaml -n customer-poc
oc apply -f infra/poc/monitoring/vllm-alerts.yaml -n customer-poc
```

> **운영 참고**: GPU 및 vLLM 알림 expr이 bare metric name(`DCGM_FI_DEV_GPU_TEMP > 85`, `vllm:num_requests_waiting > 10`)을 사용하여 네임스페이스 필터가 없다. 멀티네임스페이스 환경에서는 다른 네임스페이스의 메트릭이 혼입될 수 있으므로, 프로덕션 전환 시 `{namespace="customer-poc"}` 라벨 셀렉터를 추가해야 한다.
...
```
$ oc get prometheusrules -n customer-poc --no-headers
poc-gpu-alerts   22d
vllm-alerts      22d
...
> 📸 재촬영 필요: [OpenShift 콘솔 > Observe > Metrics] [vllm:num_requests_running PromQL 조회 결과가 그래프로 표시되는 화면] [https://console-openshift-console.apps.poc.customer.com/monitoring/query-browser 접근]

> 📸 재촬영 필요: [OpenShift 콘솔 > Observe > Alerting > Alerting Rules] [maas-token-alerts-v2, poc-gpu-alerts 규칙이 표시되는 화면] [https://console-openshift-console.apps.poc.customer.com/monitoring/alertrules 접근]
...
| `bge-m3-v1-kserve-*` (x2) | 1+1 | Pending | customer-poc |
...
| TokenRateLimitPolicy | customer-poc | `maas-trlp-redhataiqwen35-...` | Enforced |
| TokenRateLimitPolicy | customer-poc | `fallback-trlp` | Enforced |
...
| PrometheusRule | customer-poc | `poc-gpu-alerts`, `vllm-alerts` | 22d |
| ServiceMonitor | customer-poc | 12개 (모델별 메트릭 수집) | 활성 |
