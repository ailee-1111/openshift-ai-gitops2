...
NemoGuardrails Pod (nemo-kr-test-676b58f9fd-*)          <- nemoguardrails NS
  ...
  |
  v (통과 시)
Service: gemma-4-31b-it-rh-predictor.customer-poc.svc       (Headless, targetPort 8080)
  |
  v
vLLM Pod (gemma-4-31b-it-rh)                            <- customer-poc NS, GPU, port 8080
  |
  v
...
5. **의존 관계**: vLLM 모델 서빙(`gemma-4-31b-it-rh`)이 `customer-poc` NS에서 Running 상태여야 함
...
spec:
  ...
  - name: MAIN_MODEL_BASE_URL
    value: http://gemma-4-31b-it-rh-predictor.customer-poc.svc.cluster.local:8080/v1  # vLLM Pod의 직접 포트(8080). Headless Service이므로 Pod 포트로 직접 연결
...
models:
  - type: main
    engine: openai
    parameters:
      openai_api_base: "http://gemma-4-31b-it-rh-predictor.customer-poc.svc.cluster.local:8080/v1"
...
1. **vLLM 모델 서빙 Running** (`gemma-4-31b-it-rh`, customer-poc NS)
...
| vLLM (`gemma-4-31b-it-rh`) | customer-poc | Running | -- | 직접 추론 정상 확인 |
