# 211 — TrustyAI Service 구성

## 목적

PoC 네임스페이스에 TrustyAIService(모델 모니터링/메트릭 수집)를 배포한다. GuardrailsOrchestrator와 LMEvalJob은 InferenceService 배포 후 `runbooks/302-guardrails.md`에서 구성한다.

## 전제 조건

- [ ] `runbooks/210-dspa.md` 완료
- [ ] `${MODEL_NS}` 환경변수 설정
...
→ `runbooks/212-mlflow.md` — MLflow Server 구성
