...
Layer 3: S3(MinIO) + Gitea             → step 6~8
Layer 4: CMA + KEDA + MAG + MailHog    → step 9~12
...
### 11. ManualApprovalGate 설치 (Layer 4)

Tekton Pipeline 승인 게이트(S2) 전제 조건.
...
### 12. RHCL (Red Hat Connectivity Link) 설치 (Layer 5)
...
### 13. MaaS (Models as a Service) 활성화 (Layer 5)
...
### 14. htpasswd IdP + RBAC 테스트 사용자 (Layer 6)
...
### 15. Observability Dashboard 구성 (Layer 6b)
...
→ `runbooks/45-gpu-stack.md` (GPU 스택 필요 시) 또는 `runbooks/60-model-serving.md` (시나리오 구축 시작)
