# 60-b — GuardrailsOrchestrator + LMEval + EvalHub 구성

## 목적

InferenceService 배포 완료 후, GuardrailsOrchestrator(PII 감지/콘텐츠 필터링), LMEvalJob(모델 평가), EvalHub(Dashboard Evaluations 백엔드)를 배포한다. TrustyAI Operator와 TrustyAIService는 `runbooks/53-trustyai.md`에서 사전 구성 완료.

## 전제 조건

- [ ] `runbooks/53-trustyai.md` 완료 (TrustyAIService Running)
- [ ] `runbooks/60-model-serving.md` 완료 (InferenceService Ready=True)
...
→ `runbooks/70-model-serving-validation.md` — S1 모델 서빙 검증
