...
| 39 | API 키 관리 및 접근 제어 | 사용량 제어 | TPM 제한 | 키별 분당 토큰 수 제한 | DS-LLM 운영/관리 | 검증 | RateLimitPolicy counter expression으로 토큰 기반 제한 구성 가능 확인 |
...
| 58 | 모니터링 및 로깅 | 사용량 리포팅 | API 키별 사용량 | 키별 요청 수, 토큰 수 집계 | DS-LLM 운영/관리 | 검증 | Usage Dashboard 정상: Total Tokens 282, Requests 13, Errors 0, Success 100%, Active Users 1. `authorized_hits/calls{user,subscription,model}` Thanos 수집 |
...
| No | 대분류 | 세부 항목 | 설명 | 상태 | 비고 |
|----|------|---------|------|------|------|
| 62 | 모니터링 및 로깅 | 비용 할당 리포트 | 부서/팀/키 단위 비용 산정 리포트 | **부분 검증** | Tekton Pipeline으로 Thanos authorized_hits 조회 → subscription→부서/팀 매핑(ConfigMap) → CSV+HTML 리포트 생성+이메일 발송 자동화. 실측: 3개 subscription, 11,382건, $97.30. IaC: `infra/poc/pipeline/cost-allocation-report.yaml`, `pipeline-cost-report.yaml`, `cost-team-mapping.yaml` |
...
| Out-of-scope | 5 | — | — | — | — |
| 부분 격상 (No.62) | 1 | 1 | 0 | 0 | 100% |
| **합계** | **85** | **80** | **0** | **0** | **100%** (80개 대상) |

## Tradeoffs (각 옵션의 장단점)

고객 요구사항 85개 중 Out-of-scope 5개를 제외한 80개 대상 (No.62 비용 할당 리포트는 Tekton Pipeline 자동화로 부분 격상). 실측 검증 69개(87%) + 절차 준비 완료(런북+IaC 작성, 클러스터 실행 대기) 5개(6%) + 아키텍처/논리적 실증 5개(6%). 모든 항목에 검증 절차가 존재하나, 런북 작성과 클러스터 실측은 구분 필요. 성능 실측치는 경량 모델(SmolLM2-135M) 기준이며 프로덕션 모델(70B+)에서는 변동 예상.
