...
| 18 | 모델 서빙 및 배포 관리 | 대형 모델 지원 | 파이프라인 병렬화 (PP) | 모델 레이어를 GPU간 파이프라인 분할 | — | 미검증 | HGX 멀티GPU 환경 필요 |
| 19 | 모델 서빙 및 배포 관리 | 대형 모델 지원 | 멀티노드 추론 | HGX 서버 여러 대를 걸쳐 단일 모델 서빙 | — | 미검증 | 단일 노드 환경, HGX 멀티노드 필요 |
...
| 32 | 오토스케일링 및 트래픽 라우팅 | 트래픽 라우터 | 로드밸런싱 전략 | Round-robin, Least-connection, GPU utilization 기반 등 | — | 부분 검증 | Gateway API + inference-pool 기반 라우팅. 전략 커스터마이징 미수행 |
| 33 | 오토스케일링 및 트래픽 라우팅 | 트래픽 라우터 | 우선순위 기반 라우팅 | API 키/사용자 등급에 따른 우선 처리 | — | 부분 검증 | AuthPolicy 4개 구성. API 키 기반 인증 차등 |
| 34 | 오토스케일링 및 트래픽 라우팅 | 트래픽 라우터 | 폴백 라우팅 | 특정 모델 장애 시 대체 모델로 라우팅 | — | 부분 검증 | HTTPRoute에 inference-pool + workload-svc 이중 backendRef |
| 35 | 오토스케일링 및 트래픽 라우팅 | 트래픽 라우터 | GPU 자원 동적 전환 | 모델 간 GPU 자원 재할당 (시간대/수요 기반) | — | 미검증 | 시간대/수요 기반 자동 재할당 미구현 |
...
| 58 | 모니터링 및 로깅 | 사용량 리포팅 | API 키별 사용량 | 키별 요청 수, 토큰 수 집계 | DS-LLM 운영/관리 | 미검증 | MaaS TP 제한 (Usage API 미응답) |
