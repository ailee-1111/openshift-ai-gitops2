...
경량 모델 단건 검증 → 프로덕션 규모로 전환.

### S1 강화: 멀티모델 운영

| 현재 | v3 |
|------|-----|
| 단일 모델 1개 | 3+ 모델 동시 등록, 태그/라벨 분류 |
| v1→v2 확인 | v1→v2 전환 중 추론 연속성 (다운타임 0) |
| CRUD 확인 | 검색/필터 기반 모델 디스커버리 |
...
|------|-----|
| 승인 1건 | 승인 + 반려 분기 검증 |
| 성공만 확인 | Task 실패 → 알림 + 재실행 |
| 단일 파이프라인 | 등록→검증→승인→배포→검증 5단계 |

### S3 강화: GPU 메트릭 실 스케일링

| 현재 | v3 |
|------|-----|
| CPU 기반 HPA | GPU VRAM/큐 깊이 KEDA 트리거 |
| 1→3 | 1→3→1 사이클 (업+다운+안정화) |
| 단순 부하 | 계단형 부하 (10→50→100 RPS) |

### S4 강화: Chaos Engineering

| 현재 | v3 |
|------|-----|
| Pod 삭제 1회 | 연속 삭제 3회 + 복구 시간 일관성 |
| drain 1회 | drain+uncordon+재배치 사이클 |
| 미검증 | NetworkPolicy 차단 → 격리 검증 |

### S5 강화: 자동 복원 사이클

| 현재 | v3 |
|------|-----|
| Pod 0 확인 | 축소→요청→복원→추론→재축소 전체 사이클 |
| 단일 측정 | 5회 반복 → 평균/분산 |
| 135M만 | 8B(qwen3-8b) Cold Start |

### S6 강화: 알림 E2E + 감사 추적

| 현재 | v3 |
|------|-----|
| Rule 생성 확인 | CPU 90% 실제 트리거 → 알림 수신 |
| Audit Log 존재 | 모델 배포 이벤트 → Audit 추적 E2E |
| Perses 3개 | 이상 감지 → 조치 워크플로우 |
...
```
Phase E1: S1~S6 기존 시나리오 강화 런북
Phase E2: S7~S10 신규 시나리오 런북 + IaC
Phase F: 프레임워크 (overlay, scripts, .env.example)
Phase G: 클러스터 실행 (S1~S6 강화 + S7~S10 E2E)
Phase H: RTM/리포트 갱신
```
